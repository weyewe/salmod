class SalesInvoicePdf < Prawn::Document
  ...
  def title
    bounding_box( [150,cursor], :width => @page_width - 300) do
      text "Sales Invoice: #{@invoice.code}", 
          :size => 15, 
          :align => :center 
  ...
  def subscription_details
    move_down 40
    total_active_sales_entries = @invoice.active_sales_entries.count 
  ...
  def subscription_item_rows
    ...
    [[
      ...
      " ", '',
    "Total  ",  
    "#{precision(@invoice.total_amount_to_be_paid)}"
    ]]
  end
